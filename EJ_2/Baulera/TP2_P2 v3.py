...
def segmentar_caracteres(placa_img):
    ...
    AJUSTE FINO: Binarización Adaptativa y Tolerancia estadística.
    MEJORA: Se reduce la dilatación para evitar fusión de caracteres.
    """
    ...
    # 3. Operación Morfológica: AJUSTE, reducimos iteraciones
    kernel = np.ones((3, 3), np.uint8)
    thresh = cv2.dilate(thresh, kernel, iterations=1) # <-- Iteraciones 1 (antes 2)
    ...
    if caracteres_segmentados:
        ...
        fig.suptitle('B. Caracteres Segmentados (Corregido)')

        if num_caracteres == 1: axes = [axes] # Manejar caso de un solo subplot
