...
def segmentar_monedas_dados(ruta_imagen):
    ...
    # Crear elementos estructurantes más pequeños para evitar fusión
    ...
    img_opening = cv2.morphologyEx(img_thresh, cv2.MORPH_OPEN, kernel_opening, iterations=1)
    
    # Closing suave para cerrar pequeños huecos SIN fusionar objetos
    img_closing = cv2.morphologyEx(img_opening, cv2.MORPH_CLOSE, kernel_closing, iterations=1)
    
    print("  ✓ Opening aplicado (kernel elíptico 3x3, 1 iteración) - elimina ruido")
    print("  ✓ Closing aplicado (kernel elíptico 5x5, 1 iteración) - cierra huecos")
    print("  ⚠ Kernels reducidos para evitar fusión de objetos cercanos")
    ...
    # Calcular área mínima basada en el tamaño de la imagen (más permisivo)
    area_imagen = img_original.shape[0] * img_original.shape[1]
    area_minima = area_imagen * 0.0003  # 0.03% del área total (más permisivo)
    area_maxima = area_imagen * 0.15     # 15% del área total
