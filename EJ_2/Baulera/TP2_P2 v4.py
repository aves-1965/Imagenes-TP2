...
def segmentar_caracteres(placa_img):
    ...
    MEJORA: Aplicar CLAHE a la placa normalizada ANTES de binarizar.
    MEJORA: Reducir dilatación para evitar fusión.
    """
    if placa_img is None:
        print("--- 🅱️ Segmentación de Caracteres Omitida (Placa no detectada) ---")
        return

    print("\n--- 🅱️ Iniciando Segmentación de Caracteres (CON MEJORA DE CONTRASTE) ---")
    ...
    # Aplicamos CLAHE para realzar los caracteres antes de binarizar
    clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8,8))
    placa_contrast_enhanced = clahe.apply(placa_norm)
    
    # 3. Binarización Robusta (Ahora sobre la imagen con contraste mejorado)
    thresh = cv2.adaptiveThreshold(placa_contrast_enhanced, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                     cv2.THRESH_BINARY_INV, 25, 4)

    # 4. Operación Morfológica: (Ajuste de la corrección anterior)
    kernel = np.ones((3, 3), np.uint8)
    thresh = cv2.dilate(thresh, kernel, iterations=1) # Iteraciones 1
    ...
    if not alturas:
        ...
        # Opcional: Mostrar 'thresh' para depurar por qué no se encontraron
    ...
    if caracteres_segmentados:
        ...
        fig.suptitle('B. Caracteres Segmentados (Contraste Mejorado)')

        if num_caracteres == 1: axes = [axes]

        for i, char_data in enumerate(caracteres_segmentados):
            # Usamos la imagen original normalizada (placa_norm) para el recorte final,
            # no la imagen binarizada ('thresh'), para que se vea el carácter en gris.
